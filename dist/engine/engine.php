...
switch ($do) {
	...
	if ($do == "cat" and $category != '' and $subaction == '') {
		...
		if ($config['allow_alt_url']) $canonical = $url_page . "/";
		else $canonical = $PHP_SELF . "?do=cat&category=" . $cat_info[$category_id]['alt_name'];
		if ($config['allow_multi_category']) {
		...
		if ($config['allow_alt_url']) $canonical = $url_page . "/";
		else $canonical = $PHP_SELF . "?do=lastnews";
		...
		if ($config['allow_alt_url']) $canonical = $url_page . "/";
		else $canonical = $PHP_SELF . "?do=tags&tag=" . $encoded_tag;
		...
		if ($xfname) {
			...
			if ($config['allow_alt_url']) $canonical = $url_page . "/";
			else $canonical = $PHP_SELF . "?do=xfsearch&xfname=" . $xfname . "&xf=" . urlencode(str_replace("&#039;", "'", $xf));
		} else {
			...
			if ($config['allow_alt_url']) $canonical = $url_page . "/";
			else $canonical = $PHP_SELF . "?do=xfsearch&xf=" . urlencode(str_replace("&#039;", "'", $xf));
			$xfields = xfieldsload();
			$xfields_name = array();
		...
		if ($config['allow_alt_url']) $canonical = $url_page . "/";
		else $canonical = $PHP_SELF . "?subaction=allnews&user=" . urlencode($user);
		if ($view_template == "rss") {
		...
		if ($config['allow_alt_url']) $canonical = $url_page . "/";
		else $canonical = $PHP_SELF . "?subaction=newposts";
		...
		if ($config['allow_alt_url']) $canonical = $url_page . "/";
		else $canonical = $PHP_SELF . "?catalog=" . urlencode($catalog);
		...
		if ($year != '' and $month == '' and $day == '') {
			...
			if ($config['allow_alt_url']) $canonical = $url_page . "/";
			else $canonical = $PHP_SELF . "?year=" . $year;
		...
		if ($year != '' and $month != '' and $day == '') {
			...
			if ($config['allow_alt_url']) $canonical = $url_page . "/";
			else $canonical = $PHP_SELF . "?year=" . $year . "&month=" . $month;
			if (isset($_SESSION['dle_sort_date'])) $news_sort_by = $_SESSION['dle_sort_date'];
			if (isset($_SESSION['dle_direction_date'])) $news_direction_by = $_SESSION['dle_direction_date'];
		...
		if ($year != '' and $month != '' and $day != '' and $subaction == '') {
			...
			if ($config['allow_alt_url']) $canonical = $url_page . "/";
			else $canonical = $PHP_SELF . "?year=" . $year . "&month=" . $month . "&day=" . $day;
			if (isset($_SESSION['dle_sort_date'])) $news_sort_by = $_SESSION['dle_sort_date'];
			if (isset($_SESSION['dle_direction_date'])) $news_direction_by = $_SESSION['dle_direction_date'];
    ...
    */
    ...
    // TODO: Отключил автоматическую генерацию OG тегов и news_keywords в полных новостях
    ...
    // 		}
   	// 	}
    // }
    // TODO: Отключил автоматическую генерацию OG тегов и news_keywords в полных новостях END
    ...
    // TODO мой блок кода для вставки своих доп полей здесь

if ($dle_module == "showfull") {
	$myxfdata = xfieldsdataload($myxfields);
	if ($myxfdata['soc-image'] != ' ') {
		$xfield_soc_image = $myxfdata['soc-image'];
	}
	if ($xfield_soc_image != '') {
		$metatags['soc-image'] = $xfield_soc_image;
	}
}

if ($dle_module !== 'main' && $dle_module !== 'cat' && $dle_module !== 'tags') {
	$metatags = <<<HTML
	...
	<meta property="og:image" content="{$config['http_home_url']}templates/AndrewTheme/{$metatags['soc-image']}">
	<meta property="og:image:alt" content="{$metatags['title']}">
	<meta name="twitter:card" content="summary_large_image">
	<meta name="twitter:domain" content="movie-town.ru">
	<meta name="twitter:url" content="{$canonical}">
	<meta name="twitter:site" content="@AVD_SF">
  	<meta name="twitter:creator" content="@AVD_SF">
	<meta name="twitter:title" content="{$metatags['title']}">
	<meta name="twitter:description" content="{$metatags['description']}">
	<meta name="twitter:image:src" content="{$config['http_home_url']}templates/AndrewTheme/{$metatags['soc-image']}">
	<meta name="robots" content="index, noarchive">
	<meta name="google" content="notranslate">
	<meta name="yandexbot" content="noyaca">
...
if ($dle_module === 'main') {
	...
	<meta property="og:url" content="{$canonical}">
	<meta property="og:title" content="{$metatags['title']}">
	<meta property="og:image" content="{$config['http_home_url']}templates/AndrewTheme/images/index-soc-img.png">
	<meta property="og:image:alt" content="{$metatags['title']}">
	<meta name="twitter:card" content="summary_large_image">
	<meta name="twitter:domain" content="movie-town.ru">
	<meta name="twitter:url" content="{$canonical}">
  ...
  <meta name="twitter:creator" content="@AVD_SF">
	<meta name="twitter:title" content="{$metatags['title']}">
	<meta name="twitter:description" content="{$metatags['description']}">
	<meta name="twitter:image:src" content="{$config['http_home_url']}templates/AndrewTheme/images/index-soc-img.png">
	<meta name="robots" content="index, noarchive">
  <meta name="google" content="notranslate">
  <meta name="yandexbot" content="noyaca">
  {$s_meta}
...
//  TODO END мой блок кода для вставки своих доп полей здесь END
    if ($canonical) {
