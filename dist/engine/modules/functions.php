...
function build_js($js, $config) {
	...
	if ($config['js_min']) {
		...
		$js_array[] = "<script src=\"{$config['http_home_url']}engine/classes/min/index.php?g=general{$ver}&amp;v=27\"></script>";

		$default_array = array (
			// TODO: выпилил jQuery UI
		...
		$js_array[] = "<script src=\"{$config['http_home_url']}engine/classes/min/index.php?f=".implode(",", $js)."&amp;v=27\" defer></script>";
		...
		foreach ($js as $value) {
			if($i > 0) $defer =" defer";
			$js_array[] = "<script src=\"{$config['http_home_url']}{$value}?v=27\"{$defer}></script>";
			$i++;
		}
...
function build_css($css, $config) {
	...
	if ($config['js_min'] AND count($css) ) {
		...
		return "<link href=\"{$config['http_home_url']}engine/classes/min/index.php?f=".implode(",", $css)."&amp;v=27\" rel=\"stylesheet\" type=\"text/css\">";
		...
		foreach ($css as $value) {
			...
			$css_array[] = "<link href=\"{$config['http_home_url']}{$value}?v=27\" rel=\"stylesheet\" type=\"text/css\">";
