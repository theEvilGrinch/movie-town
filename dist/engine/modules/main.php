...
", "", $xfieldsdata[$value[0]])))));
            ...
            if (stripos($tpl->copy_template, "[hide") !== false) {
                $tpl->copy_template = preg_replace_callback("#\[hide(.*?)\](.+?)\[/hide\]#is", function ($matches) use ($member_id, $user_group, $lang) {
...
HTML;
    $tpl->result['content'] = preg_replace_callback("#slideshowGroup\: '(.+?)'#", function ($matches) {
    ...
    $tpl->copy_template = preg_replace_callback("#slideshowGroup\: '(.+?)'#", function ($matches) {
        global $onload_scripts;
        $matches[1] = totranslit(trim($matches[1]));
        $onload_scripts[$matches[1]] = "hs.addSlideshow({slideshowGroup: '{$matches[1]}', interval: 4000, repeat: false, useControls: true, fixedControls: 'fit', overlayOptions: { opacity: .75, position: 'bottom center', hideOnMouseOut: true } });";
        return $matches[0];
...
if ($allow_comments_ajax AND ($config['allow_quick_wysiwyg'] == "2" OR $config['allow_comments_wysiwyg'] == "2") AND $dle_module != "addnews") {
 ...
 // TODO отключил лишние скрипты и стили
...
$js_array = build_css($css_array, $config) . "" . build_js($js_array, $config);
...
if (count($onload_scripts)) {
    $onload_scripts = implode("", $onload_scripts);
    $ajax.= <<<HTML
...
$tpl->global_clear();
$db->close();
// TODO: Убрал стандартный копирайт триальной DLE
GzipOut();
?>